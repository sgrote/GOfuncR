...
See [below](#other_db) for examples how to use other packages.
...
Besides using bioconductor's annotation packages for the mapping of genes to GO-categories, it is also possible to provide the annotations directly as a dataframe wit two columns: (1) genes and (2) GO-IDs.

```{r,echo=FALSE}
custom_anno = get_anno_categories(input_hyper_bg[,1])
# to have several genes in head
custom_anno = custom_anno[6:nrow(custom_anno),]
...
```
```{r,eval=FALSE} 
## prepare some candidate genes input
gene_ids = c('NCAPG', 'APOL4', 'NGFR', 'NXPH4', 'C21orf59', 'CACNG2', 'AGTR1',
    'ANO1', 'BTBD3', 'MTUS1', 'CALB1', 'GYG1', 'PAX2')
input_hyper = data.frame(gene_ids, is_candidate=1)
## run enrichment with custom annotations
...
A default GO-graph (obtained from [geneontology](http://archive.geneontology.org/latest-termdb/), last modified on 2018-04-10), is integrated in the package.
...
[term.txt](http://www.geneontology.org/page/go-mysql-database-schema#go-graph.table.term),
[term2term.txt](http://www.geneontology.org/page/go-mysql-database-schema#go-graph.table.term2term) and
[graph_path.txt](http://www.geneontology.org/GO.database.schema.shtml#go-optimisations.table.graph-path).  
Specific versions of the GO-graph can be downloaded from [http://archive.geneontology.org/termdb/].
For example, to use the GO-graph from 2018-03-30, download and unpack the files from 
[http://archive.geneontology.org/termdb/2018-03-30/go_daily-termdb-tables.tar.gz].
Assume the files were saved in `/home/user/go_graphs/2018-03-30/`. This directory now contains the needed files `term.txt`, `term2term.txt` and `graph_path.txt` and can be used in `go_enrich`:

```{r,eval=FALSE} 
## run enrichment with custom GO-graph
go_path = '/home/user/go_graphs/2018-03-30/'
