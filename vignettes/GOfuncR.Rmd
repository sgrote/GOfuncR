...
`annotations`| NULL | optional dataframe with GO-annoations (overrides `organismDb` and `orgDb`)
...
See below for examples how to use [other packages](#other_db) or provide [custom annoations](#cu_anno).
...
[graph_path.txt](http://www.geneontology.org/GO.database.schema.shtml#go-optimisations.table.graph-path). The full path to this directory needs to be defined in the parameter `godir`.  
