...
#	gene_ids = c('NCAPG', 'QUATSCH1', 'NXPH4', 'C21orf59', 'CACNG2', 'AGTR1', 'ANO1', 'BTBD3', 'MTUS1', 'CALB1', 'GYG1', 'PAX2')
	...
	### corner cases
	# one gene
	test_results[["res2"]] = go_enrich(genes[1], n_randsets=100, silent=T)
	### mouse
	gene_ids = c('Arsi', 'Mapk4', 'Papola', 'Tfrc', 'Bak1', 'Fopnl', 'Mus81', 'Opa3', 'Npcd')
	genes = rep(1, length(gene_ids))
	names(genes) = gene_ids
	test_results[["res_mouse"]] = go_enrich(genes, n_randsets=100, ref_genome='grcm38', silent=T)
	...
	test_results[["res3"]] = go_enrich(genes, n_randsets=100, silent=T)
	### corner cases
	# one background gene
	test_results[["res4"]] = go_enrich(genes[1:(length(candi_ids)+1)], n_randsets=100, silent=T)
	# one candidate gene
	test_results[["res5"]] = go_enrich(genes[c(1,(length(candi_ids)+1):length(genes))], n_randsets=100, silent=T)
	...
	names(genes) = gene_ids
	test_results[["go_willi1"]] = go_enrich(genes, test='wilcoxon', n_randsets=100, silent=T)
	### corner cases
	# negative values
	genes_rev=-genes
	test_results[["go_willi2"]] = go_enrich(genes_rev, test='wilcoxon', n_randsets=100, silent=T)
	# only two genes
	test_results[["go_willi3"]] = go_enrich(genes[1:2], test='wilcoxon', n_randsets=100, silent=T)
	# only two scores
	genes = sample(1:2, length(gene_ids), replace=TRUE)
	names(genes) = gene_ids
	test_results[["go_willi4"]] = go_enrich(genes, test='wilcoxon', n_randsets=100, silent=T)
	...
	names(genes) = gene_ids
	test_results[["go_willi6"]] = go_enrich(genes, test='wilcoxon', n_randsets=100, silent=T)
	...
	genes = rep(1, length(gene_ids))
	names(genes) = gene_ids
	test_results[["res_mouse_len"]] = go_enrich(genes, gene_len=TRUE, n_randsets=100, ref_genome='grcm38', silent=T)
	...
	test_results[["go_region"]] = go_enrich(genes, n_randsets=100, silent=T)
	### hg20
	test_results[["go_region_hg20"]] = go_enrich(genes, n_randsets=100, ref_genome='grch38', silent=T)
	### mouse
	test_results[["go_region_mouse"]] = go_enrich(genes, n_randsets=100, ref_genome='grcm38', silent=T)

	### circ_chrom
	message("circ_chrom...")
	set.seed(123)
	test_results[["go_circ1"]] = go_enrich(genes[c(1,5,6)], n_randsets=100, circ_chrom=T, silent=T)
	test_results[["go_circ1_hg20"]] = go_enrich(genes[c(1,5,6)], n_randsets=100, circ_chrom=T, ref_genome='grch38', silent=T)
	# warning about unused chromosomes
	test_results[["go_circ2"]] = go_enrich(genes[-2], n_randsets=100, circ_chrom=T, silent=T)
